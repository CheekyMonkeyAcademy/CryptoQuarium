...
asyncFishPurchase = (fishArray, userId, index = 0, deferred = Q.defer()) => {
    ...
    if (index >= fishArray.length) {
        ...
        console.log(`Purchase fish here... id: ${fishArray[index].id} - quantity requested: ${fishArray[index].quantityToBuy}`);
        let targetQuantity = fishArray[index].quantityToBuy;
        db.Fish.findOne({
            where: {
        ...
        .then((selectedFish) => {
            if (selectedFish.quantityAvailable >= targetQuantity) {
                console.log(`You are clear to purchase: ${selectedFish.species} - quantity: ${targetQuantity}`);
            }
            else {
